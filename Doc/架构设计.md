...
> 最后更新: 2026-07-15（审计同步：更新测试计数、补充缺失文件清单）
...
| `Controllers/ApiController.cs` | API 控制器基类，内置 All/Info/GetIps 等诊断端点 |
| `Http/` | HTTP 协议编解码器（HttpCodec, HttpEncoder, HttpMessage, WebSocketServerCodec, WebSocketClientCodec） |
| `RemotingServiceResolver.cs` | 多协议服务解析器（tcp/udp/http/ws/wss 自动路由） |
| `Filters/` | ControllerContext、IActionFilter 过滤器接口 |
...
| `Controllers/BaseOAuthController.cs` | OAuth 控制器基类，令牌颁发与验证 |

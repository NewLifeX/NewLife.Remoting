...
- **异步处理管线**：服务端请求处理已异步化（`ApiServer.ProcessAsync` + `ApiHandler.ExecuteAsync`），async 动作不再阻塞线程池线程；`Process`/`Execute` 保留同步包装兼容旧调用方，非 `ApiHandler` 或重写 `Execute` 的自定义 Handler 自动回退同步路径（`OnProcessAsync` 检测）
...
```csharp
// ApiServer.ProcessAsync（同步 Process 为其包装，兼容旧调用方）
Object? result = null;
IMessage? response = null;
try
{
    result = await OnProcessAsync(...);  // 可能返回 IOwnerPacket

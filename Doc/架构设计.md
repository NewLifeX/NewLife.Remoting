...
> 最后更新: 2026-07-15（模块重组：新增 M4 服务端基础，M6 指令下发/M7 自动更新/M8 重试策略依次后延）
...
│   SessionManager  CommandSession  IRetryPolicy                │
├──────────────────────────────────────────────────────────────┤
│                   传输层 (Transport)                             │
...
| **模块依赖顺序** | M1 RPC核心 → M2流式/M3元数据 → M4服务端基础 → M5客户端基类 → M6指令下发 → M7自动更新 → M8重试策略，后序模块依赖前序模块 |
...
| `ApiClient.cs` | M1-7 | RPC 客户端，连接管理、请求/响应、流式调用、Headers 注入、重试 |
| `ApiServer.cs` / `ApiHost.cs` | M1-8 | RPC 服务端，多协议监听、请求处理流水线、流式推送 |
| `ICluster.cs` / `ClientSingleCluster` / `ClientPoolCluster` | M1-9 | 集群接口与实现 |
| `ApiNetServer.cs` / `ApiHttpServer.cs` | M1-8 | 网络传输层服务端实现 |
| `IApi.cs` / `IApiServer.cs` / `IApiSession.cs` | M1 | 核心接口契约 |
| `Controllers/ApiController.cs` | M4-1 | API 诊断控制器，内置 All/Info/GetIps 等端点 |
| `Clients/ClientBase.cs` | M5 | 客户端基类，登录/心跳/命令/WS/SSE/升级完整生命周期 |
| `Clients/WsChannel.cs` / `SseChannel.cs` | M5-2/M5-3 | WS/SSE 传输通道 |
...
| `Clients/Upgrade.cs` | M7 | 自动更新：下载/解压/覆盖/回滚/重启 |
| `IRetryPolicy.cs` | M8 | 重试策略接口 |

### 2.2 NewLife.Remoting.Extensions（扩展库）

| 目录/文件 | 模块 | 职责 |
|-----------|:----:|------|
| `Controllers/BaseController.cs` | M4-2 | 基础控制器，令牌解码/鉴权/DeviceContext 管理/异常收敛 |
| `Controllers/BaseOAuthController.cs` | M4-3 | OAuth 控制器基类，令牌颁发与刷新 |
| `Controllers/BaseDeviceController.cs` | M4-4 | 设备控制器：Login/Logout/Ping/Upgrade/Notify/NotifySSE/CommandReply/SendCommand/PostEvents |
| `Services/IDeviceService.cs` | M4-5 | 设备服务接口 |
| `Services/DefaultDeviceService.cs` | M4-6 | 设备服务默认实现 |
| `Services/TokenService.cs` | M4-7 | JWT 令牌服务 |
...
| `ApiFilterAttribute.cs` | M4 | API 过滤器，JWT 验证 |
...
**关键设计点**：
- **ApiController**（M4-1）：核心库内置诊断控制器，自动注册，提供 All/Info/GetIps 端点
- **BaseController**（M4-2）：所有扩展控制器的基类，管理 DeviceContext、令牌解码与验证、异常收敛处理
- **BaseOAuthController**（M4-3）：支持 OAuth 标准令牌颁发与刷新流程
- **BaseDeviceController**（M4-4）：设备管理核心控制器，涵盖设备完整生命周期（登录→心跳→命令→升级→通知）
- **IDeviceService**（M4-5）：设备服务抽象，可被第三方重写实现自定义设备逻辑
- **DefaultDeviceService**（M4-6）：提供默认设备管理实现，派生类可重写特定方法
- **TokenService**（M4-7）：JWT 令牌服务，区分应用令牌/设备令牌，支持自动续期
...
| 全量回归 | `dotnet test` | 631 测试通过（631 总，2 跳过），XUnitTest.Samples 另含 18 项端到端集成测试 |
...
| 2026-07-15 | 模块体系重组：新增 M4 服务端基础（含 ApiController/BaseController/BaseDeviceController/TokenService/IDeviceService）；M5 客户端基类/M6 指令下发/M7 自动更新/M8 重试策略依次排布，明确模块依赖顺序 |

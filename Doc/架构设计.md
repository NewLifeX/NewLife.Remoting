...
> 最后更新: 2026-07-15（模块重编号：M1→M2/M3→M4→M5→M6→M7→M8）
...
| **模块依赖顺序** | M1 RPC核心 → M2流式/M3元数据 → M4重试策略 → M5自动更新 → M6客户端基类 → M7指令下发 → M8服务端基础，后序模块依赖前序模块 |
...
| `Controllers/ApiController.cs` | M8-1 | API 诊断控制器，内置 All/Info/GetIps 等端点 |
| `Clients/ClientBase.cs` | M6 | 客户端基类，登录/心跳/命令/WS/SSE/升级完整生命周期 |
| `Clients/WsChannel.cs` / `SseChannel.cs` | M6-2/M6-3 | WS/SSE 传输通道 |
| `Services/SessionManager.cs` / `CommandSession.cs` | M7 | 会话管理器、命令会话基类 |
...
| `Clients/Upgrade.cs` | M5 | 自动更新：下载/解压/覆盖/回滚/重启 |
| `IRetryPolicy.cs` | M4 | 重试策略接口 |

### 2.2 NewLife.Remoting.Extensions（扩展库）

| 目录/文件 | 模块 | 职责 |
|-----------|:----:|------|
| `Controllers/BaseController.cs` | M8-2 | 基础控制器，令牌解码/鉴权/DeviceContext 管理/异常收敛 |
| `Controllers/BaseOAuthController.cs` | M8-3 | OAuth 控制器基类，令牌颁发与刷新 |
| `Controllers/BaseDeviceController.cs` | M8-4 | 设备控制器：Login/Logout/Ping/Upgrade/Notify/NotifySSE/CommandReply/SendCommand/PostEvents |
| `Services/IDeviceService.cs` | M8-5 | 设备服务接口 |
| `Services/DefaultDeviceService.cs` | M8-6 | 设备服务默认实现 |
| `Services/TokenService.cs` | M8-7 | JWT 令牌服务 |
| `Services/WsCommandSession.cs` | M7-6 | WebSocket 命令会话 |
| `Services/SseCommandSession.cs` | M7-7 | SSE 命令会话（降级通道） |
| `ModelBinders/InterfaceModelBinder.cs` | — | 接口模型绑定器，DI 容器解析接口参数 |
| `RemotingExtensions.cs` | — | AddRemoting/UseRemoting ASP.NET Core 集成扩展 |
| `ApiFilterAttribute.cs` | M8 | API 过滤器，JWT 验证 |
...
**关键设计点**：
- **ApiController**（M8-1）：核心库内置诊断控制器，自动注册，提供 All/Info/GetIps 端点
- **IDeviceService**（M8-5）：设备服务抽象，可被第三方重写实现自定义设备逻辑
- **DefaultDeviceService**（M8-6）：提供默认设备管理实现，派生类可重写特定方法
- **TokenService**（M8-7）：JWT 令牌服务，区分应用令牌/设备令牌，支持自动续期
- **BaseController**（M8-2）：所有扩展控制器的基类，管理 DeviceContext、令牌解码与验证、异常收敛处理
- **BaseOAuthController**（M8-3）：支持 OAuth 标准令牌颁发与刷新流程
- **BaseDeviceController**（M8-4）：设备管理核心控制器，涵盖设备完整生命周期（登录→心跳→命令→升级→通知），内部创建 WsCommandSession/SseCommandSession，使用 M7 ISessionManager 进行命令下发与响应等待

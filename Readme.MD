...
| 文档 | 说明 |
|------|------|
| [Doc/需求文档.md](Doc/需求文档.md) | 需求定义：愿景、功能需求（M1~M8）、非功能需求、暂缓清单 |
| [Doc/架构设计.md](Doc/架构设计.md) | 架构设计：分层概览、核心模块架构、内存管理、测试策略 |
| [Doc/功能清单.md](Doc/功能清单.md) | 功能追踪：61 个功能点的实现/测试/注释状态 |
| [Doc/SRMP.MD](Doc/SRMP.MD) | SRMP 协议规范：消息结构、通信方式、RPC 报文格式 |
| [Doc/RPC内存管理设计.md](Doc/RPC内存管理设计.md) | 零拷贝内存管理：IOwnerPacket 所有权转移与级联释放 |
| [Doc/自动更新机制.md](Doc/自动更新机制.md) | 自动更新：检测→下载→校验→解压→覆盖→重启全链路 |
...
### 元数据传递（Headers）
`ApiClient.Headers` 字典在每次调用时自动注入到参数字典，服务端可通过方法参数直接接收，或通过 `ControllerContext.Current.Parameters` 访问：
```csharp
// 客户端
client.Headers["TenantId"] = "tenant-01";
var rs = await client.InvokeAsync<String>("My/Ping", new { name = "dev" });

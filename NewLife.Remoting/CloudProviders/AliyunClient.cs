...
public class AliyunClient : ApiHttpClient
{
    ...
    /// <returns></returns>
    private static String CreateCanonicalRequest(HttpRequestMessage request, IDictionary<String, Object?>? parameters, List<String> signedHeaders)
    {
        ...
        if (request.Method == HttpMethod.Get && parameters != null)
            canonicalQuery = (String?)String.Join("&", parameters.OrderBy(p => p.Key)
